...
const Search = ({ limit }) => {
  ...
  const handleSubmit = (evt) => {
    evt.preventDefault();
    handleSearch(values.search, false);
};
...
const handleSearch = (searchValue, extend = true) => {
  // Ограничение поиска по одинаковым запросам при сабмите формы
  // Если инпут валиден и (предыдущий запрос не равен текущему или нужно добавить еще гифок по тому же запросу)
  if (isValid && (lastSearchString !== values.search || extend)) {
    ...
    api
      .searchGifs(searchValue, limit, offset)
      .then((recievedGifs) => {
        const newGifs = [
          ...(extend ? gifs : []),
...
useEffect(() => {
  if (offset !== 0) {
    handleSearch(values.search, true);
  }
    ...
    <div className="search__container">
      <form action="" className="search__form" onSubmit={handleSubmit}>
        <div className="search__input-wrapper">
          ...
          />
          <p
            className={`search__input-error ${errors.search ? "" : "search__input-error_hidden"
              }`}
          >
            {errors.search}
          </p>
